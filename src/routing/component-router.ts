...
export interface ComponentRouter {
	handleComponent(bot: Bot, interaction: APIMessageComponentInteraction): Promise<void>;
	handleModal(bot: Bot, interaction: APIModalSubmitInteraction): Promise<void>;
}
...
export function createComponentRouter(
	...
	return {
		async handleComponent(bot, interaction) {
			...
			if (isButton) {
				for (const def of buttons) {
					...
					if (params) {
						const ctx = createButtonContext(bot, interaction, params);
				...
				for (const def of selectMenus) {
					const params = matchCustomId(def.customId, customId);
					if (params) {
						const ctx = createSelectMenuContext(bot, interaction, params);
						if (!(await runBeforeInteraction(hooks, ctx))) return;
						try {
							await runHandler(ctx, () => def.handler(ctx), def.hooks?.onError);
						} finally {
		...
		async handleModal(bot, interaction) {
			...
			for (const def of modals) {
				const params = matchCustomId(def.customId, customId);
				if (params) {
					const ctx = createModalContext(bot, interaction, params);
					if (!(await runBeforeInteraction(hooks, ctx))) return;
					try {
						await runHandler(ctx, () => def.handler(ctx), def.hooks?.onError);
					} finally {
